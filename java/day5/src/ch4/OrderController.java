...
public class OrderController {
	...
	private void addItems() {
		...
		System.out.println("주문가능수");
		int qty = sc.nextInt();
		System.out.println("현재재고");
		int current = sc.nextInt();

		
		service.saveAllItems(name, price, qty, current);
		System.out.println("출력 완료");
	}
	
	private void outAllItems() {
		
		List<OrderDTO> order = service.getAllItem();
	...
	private void outItemsById() {
		...
		int p_id = sc.nextInt();
		OrderDTO item = service.getItemById(p_id);
		if (item != null) {
	...
	private void updateItems() {
	    ...
	    OrderDTO item = service.getItemById(m_id);
	    ...
	    int qty = sc.nextInt();
	    System.out.print("현재 재고(" + item.getCurrent() +"): ");
	    int current = sc.nextInt();

	    if (service.modifyItem(item, name, price, qty, current)) {
